    ...
    "{lessonId}": {
      ...
      "students": {
        "{anonymousId}": {
          ...
          "currentCodeArrangeSlots": "object | null (code_arrange student's live tile placements, written on every placement whether or not watched — { slotId: fragmentId, ... }, same shape CodeArrangeTask uses locally; updated on every tile move, independent of currentCode/currentFiles which only sync once the arrangement is fully assembled)",
...
- When watched, Python: `currentCode` per keystroke, `currentOutput` during run (leading + trailing 200ms throttle via `createThrottledMirrorWriter` in `src/app/throttledMirrorWriter.js`, so the tail of a burst is never dropped), `currentSelection`, `currentActivity`, and while an `input()` prompt is pending, `currentInputPrompt`/`currentInput` per keystroke of the not-yet-submitted answer (`writeStudentInputState`) — cleared on submit or when the prompt otherwise clears. "Watched" is re-checked on every mirror write during a run (not captured at run start), so opening `StudentModal` mid-run starts the stream and closing it stops it. Run start clears `currentOutput`/`currentInputPrompt` in one update; a prompt appearing and an `input()` submit each write the prompt state and the full output (including the echoed answer) in one `writeStudentInputState(..., { output })` update. The watch-start effect also publishes any already-pending prompt and typed value.
