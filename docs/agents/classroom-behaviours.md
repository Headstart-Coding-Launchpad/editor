...
- The student grid (`StudentCard.jsx`) shows a "👀 Info + Blocks + Stage"-style badge summarising which of these panes a student can currently see, live-synced to `sessions/{lessonId}/students/{anonymousId}/visiblePanes` via `writeStudentPresence`. This only applies to Scratch — every other lesson type's explainer and code panes are always both on screen at once (`SplitPane` on desktop, stacked-and-scrollable on mobile), so there's nothing to report there. `ScratchWorkspace.jsx` derives its own `['blocks']`/`['stage']`/`['blocks','stage']` from `hideStage`/`compact`/`activePane`/`stagePanelCollapsed` and reports it via `onVisiblePanesChange`; `LessonTaskContent.jsx` combines that with its own Instructions/Code tab state (`taskPanelCompact`/`taskPanelTab`) into the final array and hands it to `StudentView.jsx`'s `onVisiblePanesChange`, which writes to Firebase only for a real student in a live/sandbox session (never `teacherPresentation`), deduped against the last-written value.
