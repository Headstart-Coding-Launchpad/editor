...
- The response to a shrinking panel is graduated, and — deliberately — the explainer gives way *before* the code area (Blocks/Stage) is ever forced to compact, not after. `LessonTaskContent.jsx` measures its explainer+workspace container (`useElementSize.js`, a `ResizeObserver` hook) and, for Scratch lessons only:
  ...
  3. **Stage tabs away from Blocks — the final fallback**: only if the panel is so narrow that even giving Code the *entire* width still leaves it under `ScratchWorkspace.jsx`'s own `NARROW_BREAKPOINT`/`NARROW_BREAKPOINT_HEIGHT` (1000px/600px — the height axis matters here, unlike step 2) does `ScratchWorkspace.jsx` — which independently measures its own real rendered container — switch to its own Blocks/Stage `PanelTabs` switcher, replacing the old `preferStageSidePanel`/auto-collapsing-stage-rail behaviour. No coordination code is needed for this step beyond the shared constant above — it otherwise falls out of each component measuring its own real rendered size. In the classroom flow this now only really triggers on a short-but-wide window (width alone is handled by step 2); it's unconditional for the teacher-modal `TeacherLiveView` (`forceCompact`, see below).
...
- **Gotcha #3 — a growing sibling (e.g. the completion banner) starving ScratchWorkspace's height**: `scratchTaskPanelWrap` gives its panel a real `minHeight` (`SCRATCH_CODE_WIDE_HEIGHT`, imported from `ScratchWorkspace.jsx`'s `NARROW_BREAKPOINT_HEIGHT` — same lockstep-import principle as the width threshold above), instead of `minHeight: 0`. Without it, a taller sibling (the completion banner wrapping onto more lines, say) directly shrinks the panel's available height instead of `StudentView.jsx`'s `s.body` (`overflow: 'auto'`) scrolling around it — pushing `ScratchWorkspace`'s own measured height under its compact threshold and forcing Blocks/Stage into tabs for no better reason than a banner temporarily taking up space.
  For that floor to actually take effect, **two** ancestors between the panel and `s.body` both need `minHeight: 'auto'` (not `0`, and not simply omitted) — CSS only re-enables a flex item's content-based automatic minimum size (the thing that lets it grow to fit a child's explicit `min-height`) when `overflow: visible` *and* `min-height` isn't pinned to `0`; either ancestor left at `0` clips the floor regardless of what the other one does:
  1. `LessonTaskContent.jsx`'s `transitionStyle`, applied to `TaskSlideTransition`'s outer `.task-slide-viewport` — overrides `taskContentStyle`'s baked-in `minHeight: 0` (and keeps `overflow: 'visible'`, not the `overflow: 'hidden'` the other fluid-workspace lesson types use, where `SplitPane` manages its own per-pane scrolling instead).
  2. `TaskSlideTransition.jsx`'s own `.task-slide-panel` CSS class (`index.css`) *also* hardcodes `min-height: 0` — a `style` prop only reaches the outer viewport div, not this inner one, so `TaskSlideTransition` takes a new optional `panelStyle` prop (applied to the entering panel only; `undefined` for every other caller) that `LessonTaskContent.jsx` uses to override this second occurrence for Scratch.
