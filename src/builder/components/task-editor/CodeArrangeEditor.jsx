...
export default function CodeArrangeEditor({ task, onUpdate }) {
  ...
  const [previewPyodideStatus, setPreviewPyodideStatus] = useState(
    lessonMod.runtime.isReady?.() ? 'ready' : 'idle'
  )
  ...
  function updatePart(index, partIndex, updater) {
    updateLine(index, (line) => ({
      ...line,
      parts: line.parts.map((p, i) => (i === partIndex ? updater(p) : p)),
    }))
  }
  ...
  function handleEntryFileChange(entryFile) {
    ...
    if (!files.some((f) => f.name === entryFile))
      files.unshift({ name: entryFile, type: 'html', content: '' })
  ...
  function removeStaticFile(index) {
    set(
      'starterFiles',
      (task.starterFiles ?? []).filter((_, i) => i !== index)
    )
  }
  ...
  async function handlePreviewRun() {
    ...
    const src = lessonMod.runtime.buildPreviewSrc(
      { files, entryFile: getCodeArrangeEntryFile(task) },
      task,
  ...
  const feedbackPreview =
    task.check && previewCheckAttempted
      ? evaluateFeedbackCheckResults(task, previewOutput, {
          code: previewAssembled ?? '',
  ...
  return (
    <>
      ...
      {moduleType === 'html' && (
        <Field
          label="Other files"
          hint="Static files that aren't assembled from tiles, e.g. style.css"
        >
          <div className="te-quiz-answer-stack">
            {(task.starterFiles ?? [])
              .filter((f) => f.name !== (task.entryFile ?? 'index.html'))
              .map((f) => {
                ...
                return (
                  <div key={f.name} className="te-quiz-answer-card">
                    <input
                      ...
                      style={{ maxWidth: 160 }}
                      value={f.name}
                      onChange={(e) => updateStaticFile(index, { name: e.target.value })}
                    />
                    <textarea
                      className="te-input"
                      style={caStyles.codeTextarea}
                      value={f.content ?? ''}
                      onChange={(e) => updateStaticFile(index, { content: e.target.value })}
                      spellCheck={false}
                    />
                    <button
                      type="button"
      ...
      <Field label="Completion check">
        ...
        >
          <input
            ...
            checked={!!task.check}
            onChange={(e) =>
              set('check', e.target.checked ? (lessonMod?.defaultCheck?.('run') ?? null) : null)
            }
      ...
      <TaskPreviewPanel>
        ...
        {previewCheckAttempted && (
          <div
            style={{
              ...
              fontSize: '0.88rem',
              lineHeight: 1.6,
              background: previewCheckPassed ? '#f0fdf4' : '#fffbeb',
              borderColor: previewCheckPassed ? '#bbf7d0' : '#fde68a',
            }}
          >
            {previewCheckPassed
              ? 'Check passes — students will see the completion banner.'
              : 'Check does not pass with this ordering — try another arrangement or review the check.'}
            {feedbackPreview.some((r) => r.passed) && (
              <div style={{ marginTop: 6, color: '#92400e' }}>
                Feedback check matched:{' '}
                {feedbackPreview.find((r) => r.passed)?.hint || '(no hint set)'}
              </div>
            )}
          </div>
        )}
        {!previewComplete && (
          ...
          >
            Drag tiles into every slot above, or use "Load authored solution", to test-run this
            task.
          </div>
        )}
      </TaskPreviewPanel>
...
function LinePreview({ parts }) {
  ...
  return (
    <div style={caStyles.linePreview}>
      {parts.map((part, i) =>
        part.type === 'slot' ? (
          <span key={i} style={caStyles.linePreviewSlot}>
            {part.code?.trim() ? part.code : '(blank)'}
          </span>
        ) : (
          <span key={i} style={caStyles.linePreviewText}>
            {part.text || '·'}
          </span>
        )
      )}
  ...
  return (
    <div style={caStyles.textChip}>
      ...
      <div style={caStyles.chipControls}>
        ...
        <button type="button" className="te-remove-btn" onClick={onRemove} title="Remove text">
          ✕
        </button>
      </div>
    </div>
    ...
    <div style={caStyles.slotChip}>
      <div style={caStyles.chipHeader}>
        ...
        <div style={caStyles.chipControls}>
          ...
          <button type="button" className="te-remove-btn" onClick={onRemove} title="Remove blank">
            ✕
          </button>
        </div>
      </div>
