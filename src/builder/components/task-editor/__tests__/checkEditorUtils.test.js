...
describe('subjectOpFromType', () => {
  it('maps output check types correctly', () => {
    ...
    expect(subjectOpFromType('output_not_empty')).toEqual({
      subject: 'output',
      operator: 'not_empty',
    })
    expect(subjectOpFromType('output_empty')).toEqual({ subject: 'output', operator: 'empty' })
    expect(subjectOpFromType('output_contains')).toEqual({
      subject: 'output',
      operator: 'contains',
    })
    expect(subjectOpFromType('output_equals')).toEqual({ subject: 'output', operator: 'equals' })
    expect(subjectOpFromType('output_not_contains')).toEqual({
      subject: 'output',
      operator: 'not_contains',
    })
    expect(subjectOpFromType('output_not_equals')).toEqual({
      subject: 'output',
      operator: 'not_equals',
    })
    expect(subjectOpFromType('output_matches_regex')).toEqual({
      subject: 'output',
      operator: 'matches_regex',
    })
    expect(subjectOpFromType('output_not_matches_regex')).toEqual({
      subject: 'output',
      operator: 'not_matches_regex',
    })
    expect(subjectOpFromType('output_line_count')).toEqual({
      subject: 'output',
      operator: 'line_count',
    })
  })
  ...
  it('maps code check types correctly', () => {
    ...
    expect(subjectOpFromType('code_does_not_contain')).toEqual({
      subject: 'code',
      operator: 'not_contains',
    })
    expect(subjectOpFromType('code_not_equals')).toEqual({
      subject: 'code',
      operator: 'not_equals',
    })
    expect(subjectOpFromType('code_matches_regex')).toEqual({
      subject: 'code',
      operator: 'matches_regex',
    })
    expect(subjectOpFromType('code_not_matches_regex')).toEqual({
      subject: 'code',
      operator: 'not_matches_regex',
    })
  })
  ...
  it('maps element check types correctly', () => {
    ...
    expect(subjectOpFromType('element_value')).toEqual({
      subject: 'element',
      operator: 'value_contains',
    })
    expect(subjectOpFromType('element_value_equals')).toEqual({
      subject: 'element',
      operator: 'value_equals',
    })
    expect(subjectOpFromType('element_attribute')).toEqual({
      subject: 'element',
      operator: 'attribute_equals',
    })
    expect(subjectOpFromType('element_style_property')).toEqual({
      subject: 'element',
  ...
  it('maps variable check types correctly', () => {
    expect(subjectOpFromType('variable_exists')).toEqual({
      subject: 'variable',
      operator: 'exists',
    })
    expect(subjectOpFromType('variable_equals')).toEqual({
      subject: 'variable',
      operator: 'equals',
    })
    expect(subjectOpFromType('variable_dict_key_value')).toEqual({
      subject: 'variable',
      operator: 'dict_key_value',
    })
    expect(subjectOpFromType('variable_array_nth_item')).toEqual({
      subject: 'variable',
...
describe('subjectOpFromCheck', () => {
  it('uses canonical check operators when present', () => {
    expect(subjectOpFromCheck({ type: 'output', operator: 'not_matches_regex' })).toEqual({
      subject: 'output',
      operator: 'not_matches_regex',
    })
    expect(subjectOpFromCheck({ type: 'code', operator: 'equals' })).toEqual({
      subject: 'code',
      operator: 'equals',
    })
    expect(subjectOpFromCheck({ type: 'output_line_count', operator: 'less_than' })).toEqual({
      subject: 'output',
      operator: 'line_count_less_than',
    })
  })

  it('maps canonical html element checks to builder operators', () => {
    expect(subjectOpFromCheck({ type: 'html_element', operator: 'exists' })).toEqual({
      subject: 'element',
      operator: 'exists',
    })
    expect(
      subjectOpFromCheck({ type: 'html_element_count', operator: 'greater_than_or_equal' })
    ).toEqual({ subject: 'element', operator: 'count_greater_than_or_equal' })
    expect(subjectOpFromCheck({ type: 'html_element_value', operator: 'matches_regex' })).toEqual({
      subject: 'element',
      operator: 'value_matches_regex',
    })
    expect(subjectOpFromCheck({ type: 'html_element_attribute', operator: 'not_equals' })).toEqual({
      subject: 'element',
      operator: 'attribute_not_equals',
    })
    expect(
      subjectOpFromCheck({ type: 'html_element_style_property', operator: 'contains' })
    ).toEqual({ subject: 'element', operator: 'style_contains' })
  })

  it('displays legacy count checks as canonical count equals', () => {
    expect(subjectOpFromCheck({ type: 'element_count' })).toEqual({
      subject: 'element',
...
describe('checkUiFromCheck', () => {
  it('splits canonical checks into subject, field, and condition', () => {
    expect(checkUiFromCheck({ type: 'html_element_attribute', operator: 'exists' })).toEqual({
      subject: 'element',
      aspect: 'attribute',
      operator: 'attribute_exists',
    })
    expect(
      checkUiFromCheck({ type: 'html_element_style_property', operator: 'not_equals' })
    ).toEqual({ subject: 'element', aspect: 'style', operator: 'style_not_equals' })
    expect(checkUiFromCheck({ type: 'output_line_count', operator: 'less_than_or_equal' })).toEqual(
      { subject: 'output', aspect: 'line_count', operator: 'line_count_less_than_or_equal' }
    )
    expect(checkUiFromCheck({ type: 'variable_dict_key_value' })).toEqual({
      subject: 'variable',
      aspect: 'dictionary',
      operator: 'dict_key_value',
    })
...
describe('typeFromSubjectOp', () => {
  ...
  it('legacy aliases code_does_not_contain and element_value round-trip to their canonical names', () => {
    expect(typeFromSubjectOp(...Object.values(subjectOpFromType('code_does_not_contain')))).toBe(
      'code_not_contains'
    )
    expect(typeFromSubjectOp(...Object.values(subjectOpFromType('element_value')))).toBe(
      'element_value_contains'
    )
  })
...
describe('checkFromSubjectOp', () => {
  it('writes canonical output and code checks', () => {
    expect(checkFromSubjectOp('output', 'contains', { value: 'hello', hint: 'try' })).toEqual({
      type: 'output',
      operator: 'contains',
      value: 'hello',
      hint: 'try',
    ...
    expect(
      checkFromSubjectOp('code', 'not_matches_regex', { value: 'eval\\(', flags: 'i' })
    ).toEqual({ type: 'code', operator: 'not_matches_regex', value: 'eval\\(', flags: 'i' })
  })

  it('writes canonical line count checks', () => {
    expect(checkFromSubjectOp('output', 'line_count_less_than', { value: '5' })).toEqual({
  ...
  it('writes canonical html element checks with their extra fields', () => {
    expect(
      checkFromSubjectOp('element', 'attribute_matches_regex', {
        selector: 'a',
        attribute: 'href',
        value: '^https:',
        flags: 'i',
...
describe('getOperatorOptions', () => {
  it('returns aspect options for split check controls', () => {
    expect(getAspectOptions('element').map((o) => o.value)).toEqual([
      'element',
      'value',
      'attribute',
      'style',
    ...
    expect(getAspectOptions('output').map((o) => o.value)).toEqual([
      'text',
      'line_count',
      'output_state',
    ])
    expect(getAspectOptions('variable').map((o) => o.value)).toEqual([
      'variable',
      'dictionary',
      'array',
    ])
  ...
  it('returns element operators scoped by aspect', () => {
    ...
    expect(getOperatorOptions('element', null, 'count').map((o) => o.value)).toContain(
      'count_less_than_or_equal'
    )
  })

  it('returns variable operators scoped by aspect', () => {
    expect(getOperatorOptions('variable', null, 'variable').map((o) => o.value)).toEqual([
      'exists',
      'type',
      'equals',
    ])
    expect(getOperatorOptions('variable', null, 'dictionary').map((o) => o.value)).toContain(
      'dict_key_value'
    )
    expect(getOperatorOptions('variable', null, 'array').map((o) => o.value)).toContain(
      'array_nth_item'
    )
  })
...
describe('makeCheckSkeleton', () => {
  ...
  it('preserves hint from prev', () => {
    expect(makeCheckSkeleton('code_no_error', { hint: 'try again' })).toEqual({
      type: 'code_no_error',
      hint: 'try again',
    })
    expect(makeCheckSkeleton('output_contains', { hint: 'check output', value: 'hello' })).toEqual({
  ...
  it('produces name+value skeleton for variable checks', () => {
    expect(makeCheckSkeleton('variable_equals')).toEqual({
      type: 'variable_equals',
      name: '',
      value: '',
    })
    expect(makeCheckSkeleton('variable_type')).toEqual({
      type: 'variable_type',
      name: '',
      value: '',
    })
  ...
  it('produces selector+value with default "1" for element_count', () => {
    expect(makeCheckSkeleton('element_count')).toEqual({
      type: 'element_count',
      selector: '',
      value: '1',
    })
  ...
  it('produces selector+attribute+value for element_attribute', () => {
    expect(makeCheckSkeleton('element_attribute')).toEqual({
  ...
  it('produces selector+property+value for element_style_property', () => {
    expect(makeCheckSkeleton('element_style_property')).toEqual({
...
describe('formatCheckFailure', () => {
  it('prefixes with "Check does not pass"', () => {
    expect(formatCheckFailure({ type: 'output_contains', value: 'hello' })).toMatch(
      /^Check does not pass/
    )
  })
  ...
  it('formats element_count failure with count and selector', () => {
    const detail = formatCheckFailureDetail({
      type: 'element_count',
      value: '3',
      selector: '.item',
    })
