...
describe('TaskPreviewPanel authoring metadata', () => {
  ...
  it('renders no authoring metadata section when no task is supplied', () => {
    ...
    expect(screen.queryByText('Authoring metadata (author-only)', { exact: false })).not.toBeInTheDocument()
  })
  ...
  it('is visible by default when the lesson is a draft', () => {
    render(
      <TaskPreviewPanel task={{ id: 1, title: 'Task', intent: 'Teach **recursion**.', taskActivity: 'Whiteboard demo' }} draft>
        <div>Student-facing content</div>
      </TaskPreviewPanel>
    )
  ...
  it('collapses by default when the lesson is not a draft, and expands on click', () => {
    render(
      <TaskPreviewPanel task={{ id: 1, title: 'Task', intent: 'Teach recursion.', taskActivity: 'Whiteboard demo' }} draft={false}>
        <div>Student-facing content</div>
      </TaskPreviewPanel>
    )
