...
describe('PredefinedBlocksEditor', () => {
  it('clears an in-progress block when the toolbox changes', async () => {
    ...
    const { rerender } = render(
      <PredefinedBlocksEditor predefinedBlocks={[]} toolbox={motionToolbox} onChange={onChange} />
    )
    ...
    rerender(
      <PredefinedBlocksEditor predefinedBlocks={[]} toolbox={looksToolbox} onChange={onChange} />
    )
    ...
    expect(onChange).toHaveBeenCalledWith([expect.objectContaining({ type: 'looks_say' })])
  })
})
...
describe('PrebuiltStacksEditor', () => {
  it('adds a stack immediately when a block type is selected from the picker', () => {
    const onChange = vi.fn()
    render(<PrebuiltStacksEditor prebuiltStacks={[]} onChange={onChange} />)
