...
export function TargetedStageOfferPreview({ task, lessonType, incorrectCheckResults }) {
  ...
  const stage = Number.isInteger(Number(offer?.stageIndex)) ? task?.codeStages?.[Number(offer.stageIndex)] : null
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <CheckFeedbackBanner
        ...
        stageActionLabel={isReplacement ? `Try ${stage.label || 'guided stage'}` : `Show ${stage.label || 'reference'}`}
        stageActionConfirm={isReplacement ? `This will replace your current work with “${stage.label || 'this stage'}”. Continue?` : undefined}
      />
      {shown && (
        <SupportStagePanel
          stage={stage}
          ...
          revealed
          sourceLabel={isReplacement ? 'Replacement preview — builder code is unchanged' : 'Read-only student reference'}
        />
      )}
    </div>
  )
...
export default function TaskCheckResults({ task, lessonType, checkResults, incorrectCheckResults }) {
  ...
  return (
    <>
      ...
      <TargetedStageOfferPreview task={task} lessonType={lessonType} incorrectCheckResults={incorrectCheckResults} />
    </>
  )
}
