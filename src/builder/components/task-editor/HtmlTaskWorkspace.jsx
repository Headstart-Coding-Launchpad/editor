...
export default function HtmlTaskWorkspace({
  ...
  codeStages,
  selectedFile,
  selectedCompleteFile,
  setSelectedFile,
  setSelectedCompleteFile,
  running,
  htmlPreviewOpen,
  setHtmlPreviewOpen,
  iframeSrc,
      ...
      <div className="te-code-workspace-stack">
        ...
        <div className="te-html-split">
          {isSupportStage ? (
            ...
            <div className="te-no-file">
              Add a Starter stage to begin authoring this task. Legacy starter and complete files
              remain available to existing lessons but are no longer edited here.
            </div>
          ) : (
            <SplitPane
              defaultSplit={34}
                  ...
                  <FileManager
                    ...
                    onChangeType={(name, type) => {
                      if (isCompleteTab)
                        set(
                          'completeFiles',
                          (task.completeFiles ?? []).map((f) =>
                            f.name === name ? { ...f, type } : f
                          )
                        )
                      else if (isStageTab)
                        updateStage(activeStageIndex, {
                          files: (activeStage?.files ?? []).map((f) =>
                            f.name === name ? { ...f, type } : f
                          ),
                        })
                      else
                    ...
                    <div className="te-html-editor-pane">
                      {activeSelectedFile ? (
                        <CodeEditor
                          ...
                          value={
                            activeFiles.find((f) => f.name === activeSelectedFile)?.content ?? ''
                          }
                          language={
                            activeFiles.find((f) => f.name === activeSelectedFile)?.type ?? 'html'
                          }
                          onChange={(v) => {
                            if (isCompleteTab)
                              set(
                                'completeFiles',
                                (task.completeFiles ?? []).map((f) =>
                                  f.name === activeSelectedFile ? { ...f, content: v } : f
                                )
                              )
                            else if (isStageTab)
                              updateStage(activeStageIndex, {
