...
function InputRow({ input, onChange, onRemove, canRemove }) {
  ...
  return (
    <div style={s.row}>
      ...
      />
      {canRemove && (
        <button type="button" style={s.removeBtn} onClick={onRemove} title="Remove input">
          ×
        </button>
      )}
    </div>
...
function SingleTestEditor({ test, onChange, onRemove, lessonType }) {
  ...
  function updateInput(index, updated) {
    setField(
      'inputs',
      test.inputs.map((inp, i) => (i === index ? updated : inp))
    )
  }
  function removeInput(index) {
    setField(
      'inputs',
      test.inputs.filter((_, i) => i !== index)
    )
  }
  function addInput() {
    ...
    <div style={s.card}>
      ...
      <div>
        ...
        <div style={{ ...s.hint, marginTop: 4 }}>
          Use {'{'} name {'}'} in the check value below to substitute this input's value, e.g. Hello{' '}
          {'{'} username {'}'}
        </div>
      </div>
    ...
    <Field label="Tests">
      <div style={s.wrap}>
        <p style={s.description}>
          Define test cases with pre-set inputs. Students must run all tests to complete the task —
          plain Run stays interactive and does not count for completion.
        </p>
        {tests.map((test, i) => (
          <SingleTestEditor
            key={test.id ?? i}
