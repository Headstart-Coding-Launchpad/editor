...
function validateCodeChecks(
  checks,
  n,
  errors,
  { type, interactionMode, kind = 'completion' } = {}
) {
  const label = labelCheckKind(kind)
...
export function validateLesson(lesson) {
  ...
  flat.forEach((task, i) => {
    ...
    if (
      ...
      task.informationType !== 'introduction' &&
      !task.explainer?.trim()
    ) {
    ...
    if (task.taskType === 'quiz') {
      ...
      if (task.codeStages?.length > 0) {
        task.codeStages.forEach((stage, si) => {
          ...
          if (!stage.fs || typeof stage.fs !== 'object')
            errors.push(`Task ${n} stage ${si + 1} has no filesystem state`)
        })
      }
      ...
      if (carryCircuitFrom != null && !flat.some((t) => t.id === carryCircuitFrom)) {
        errors.push(
          `Task ${n} references task ${carryCircuitFrom} for circuit carry-through but that task does not exist`
        )
      }
      if (task.check) validateElectronicsChecks(task.check, n, errors)
      if (feedbackChecks.length > 0)
        validateElectronicsChecks(feedbackChecks, n, errors, 'feedback')
      ...
      lines.forEach((line, li) => {
        ...
        parts.forEach((part, pi) => {
          ...
          if (part?.type === 'slot') {
            ...
            if (!part.code?.trim())
              errors.push(`Task ${n} line ${ln} blank ${pn} has no correct value.`)
      ...
      if (type === 'html') {
        if (!task.starterFiles || task.starterFiles.length === 0)
          ...
          if (
            !task.starterFiles.some((file) => file.type === 'html' || file.name.endsWith('.html'))
          ) {
    ...
    if (task.taskType !== 'information' && task.taskType !== 'quiz') {
      if (type === 'scratch') {
        ...
        if (feedbackChecks.length > 0)
          validateCodeChecks(feedbackChecks, n, errors, {
            type,
    ...
    if (
      ...
      !allowedCarrySourceIds.includes(carryFrom)
    ) {
    ...
    if (task.taskType !== 'information' && task.taskType !== 'quiz' && task.check) {
      ...
      if (type === 'html' && task.completeFiles?.length > 0) {
        ...
        if (staticChecks.length > 0) {
          ...
          if (staticChecks.some((c) => !evaluateSingleCheck(c, '', { code: codeStr }))) {
            warnings.push(
              `Task ${n} complete solution fails a code check — review the complete files`
            )
          }
        }
        if (dynamicChecks.length > 0 && !task._checkTested) {
      ...
      if (type === 'filesystem' && task.completeFs && typeof task.completeFs === 'object') {
        const fsChecks = allChecks.filter(
          (c) =>
            c.type?.startsWith('fs_') && c.type !== 'fs_dir_opened' && c.type !== 'fs_file_opened'
        )
        if (
          fsChecks.length > 0 &&
          fsChecks.some((c) => !evaluateSingleCheck(c, '', { fs: task.completeFs }))
        ) {
          warnings.push(
    ...
    if (
      ...
      if (
        circuitChecks.length > 0 &&
        circuitChecks.some((c) => !evaluateSingleCheck(c, '', { circuit: task.completeCircuit }))
      ) {
        warnings.push(
