...
export function wireRoutePoints(
  ...
  const obstacles = componentRects.map((rect) => ({
    ...rect,
  ...
  const outerLane = outsideFacingRoute(
    fromExit,
    toExit,
    fromRef,
    toRef,
  ...
  const points = compactPathPoints([from, ...middle, to], {
    preserveIndexes: new Set([1, middle.length]),
  })
  return points
}
...
export function outsideFacingRoute(
  ...
  const topLane = clamp(
    Math.min(start.y, end.y, ...relevant.map((rect) => rect.top)) - WIRE_CLEARANCE * 1.5,
    minY,
    maxY
  )
  const bottomLane = clamp(
    Math.max(start.y, end.y, ...relevant.map((rect) => rect.bottom)) + WIRE_CLEARANCE * 1.5,
    minY,
    maxY
  )
  const candidates = [
...
export function isTinyReversal(previous, point, next) {
  ...
  if (sameHorizontal) {
    ...
    return (
      turnsBack &&
      Math.min(Math.abs(point.x - previous.x), Math.abs(next.x - point.x)) <= WIRE_EXIT_STUB
    )
  }
  if (sameVertical) {
    ...
    return (
      turnsBack &&
      Math.min(Math.abs(point.y - previous.y), Math.abs(next.y - point.y)) <= WIRE_EXIT_STUB
    )
  }
  return false
}
...
export function routeOrthogonal(start, end, obstacles, bounds = null, usedWireSegments = []) {
  ...
  if (
    directIsOrthogonal &&
    !directHitsComponent &&
    wireLanePenalty(start, end, usedWireSegments) === 0
  )
    return [start, end]
  ...
  if (!nodeMap.has(startKey))
    nodeMap.set(startKey, { x: roundCoord(start.x), y: roundCoord(start.y) })
...
export function fallbackOrthogonalRoute(
  ...
  const leftLane = clamp(
    Math.min(start.x, end.x, ...relevant.map((rect) => rect.left)) - WIRE_CLEARANCE * 1.5,
    minX,
    maxX
  )
  const rightLane = clamp(
    Math.max(start.x, end.x, ...relevant.map((rect) => rect.right)) + WIRE_CLEARANCE * 1.5,
    minX,
    maxX
  )
  const candidates = [
    ...
    .map((points) =>
      compactPathPoints(points, { preserveIndexes: new Set([1, points.length - 2]) })
    )
    .map((points) => ({
      points,
  ...
  return segmentsFromPoints(points).reduce(
    (score, segment) => {
      return (
        ...
        wireLanePenalty(segment.a, segment.b, usedWireSegments)
      )
    },
