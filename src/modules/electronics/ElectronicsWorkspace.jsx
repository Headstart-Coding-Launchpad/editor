...
export default function ElectronicsWorkspace({
  ...
  const activeMicrocontroller =
    circuit.components.find((component) => component.type === 'microcontroller') ?? null
  ...
  const paletteTypes = useMemo(
    () => normalizeAvailableComponents(availableComponents),
    [availableComponents]
  )
  ...
  const largestPart = circuit.components.reduce(
    (largest, component) => {
  ...
  const buzzersOn = useMemo(
    () =>
      circuit.components.filter((c) => c.type === 'buzzer' && getComponentState(circuit, c.id).on)
        .length,
    [circuit]
  )
  ...
  useEffect(() => {
    ...
    function applyFit() {
      ...
      setBoardScale(
        clampBoardScale(Math.min(1, availableWidth / boardWidth, availableHeight / boardHeight))
      )
    }
  ...
  function clampComponentPosition(position, component) {
    ...
    const offsets = (component.pins?.length ? component.pins : ['a']).map((pin, index) =>
      pinOffset(component, pin, index)
    )
    ...
    offsets.forEach((offset) => {
      ...
      maxCol = Math.min(
        maxCol,
        Math.floor((lastHoleX + anchor.x - offset.x - BOARD_PAD) / GRID_X) + 1
      )
      minRow = Math.max(minRow, Math.ceil((anchor.y - offset.y) / GRID_Y) + 1)
      maxRow = Math.min(
        maxRow,
        Math.floor((lastHoleY + anchor.y - offset.y - BOARD_PAD) / GRID_Y) + 1
      )
    })
    ...
    return {
      row: clamp(
        Math.round(Number.isFinite(row) ? row : 1),
        Math.min(minRow, maxRow),
        Math.max(minRow, maxRow)
      ),
      col: clamp(
        Math.round(Number.isFinite(col) ? col : 1),
        Math.min(minCol, maxCol),
        Math.max(minCol, maxCol)
      ),
    }
  }
  ...
  function handleBoardWheel(event) {
    ...
    setZoomManually((prev) =>
      clampBoardScale(prev + (event.deltaY < 0 ? BOARD_SCALE_STEP : -BOARD_SCALE_STEP))
    )
  }
  ...
  function addComponent(type, position) {
    ...
    const fallback = {
      row: 2 + (count % Math.max(1, boardRows - 2)),
      col: 3 + (count % Math.max(1, boardCols - 4)),
    }
  ...
  function removeSelected() {
    ...
    if (selectedWire) {
      ...
      next.wires = next.wires.filter(
        (w) => !w.from.startsWith(`${selected.id}.`) && !w.to.startsWith(`${selected.id}.`)
      )
  ...
  function updateSelectedWireLocked(locked) {
    ...
    next.wires = next.wires.map((wire) =>
      wire.id === selectedWire.id ? { ...wire, locked } : wire
    )
    update(next)
  }
  ...
  function updateSelectedComponent(updates) {
    ...
    const next = cloneCircuit(circuit)
    next.components = next.components.map((component) =>
      component.id === selected.id ? { ...component, ...updates } : component
    )
    update(next)
  }
  ...
  function updateMicrocontrollerCode(nextCode) {
    ...
    const next = cloneCircuit(circuit)
    next.components = next.components.map((component) =>
      component.id === activeMicrocontroller.id
  ...
  function addGpioPin() {
    ...
    next.components = next.components.map((component) =>
      component.id === target.id ? { ...component, pins } : component
    )
  ...
  function rotateSelectedComponent() {
    ...
    updateSelectedComponent({
      rotation,
      position: clampComponentPosition(rotated.position ?? { row: 1, col: 1 }, rotated),
    })
  }
  ...
  function connectPins(from, to) {
    ...
    if (from && to && from !== to) {
      ...
      next.wires.push({
        ...
        from,
        to,
        color: getWireColorForPins(from, to, wireColor),
      })
  ...
  function startComponentDrag(event, component) {
    if (
      readOnly ||
      (component.locked && !setupMode) ||
      event.button !== 0 ||
      event.target.closest('[data-pin-ref]') ||
    ...
    setDrag({
      type: 'component',
      id: component.id,
      offset: { x: point.x - part.x, y: point.y - part.y },
    })
  ...
  function handleBoardPointerMove(event) {
    ...
    if (drag.type === 'component') {
      ...
      setDrag((current) =>
        current?.type === 'wire'
          ? { ...current, current: snap?.point ?? point, to: snap?.ref ?? null }
          : current
      )
    } else if (drag.type === 'control') {
      const deltaX = point.x - drag.startPoint.x
      const nextValue = Math.round(
        Math.min(100, Math.max(0, drag.startValue + (deltaX / CONTROL_SLIDE_RANGE_PX) * 100))
      )
  ...
  function interactivePanTarget(target) {
    return target?.closest?.(
      '[data-component], [data-pin-ref], [data-wire-hit], [data-control-action], button, input, select, textarea, a'
    )
  }

  function startBoardPan(event) {
    if (event.defaultPrevented || event.button !== 0 || drag || interactivePanTarget(event.target))
      return
  ...
  const breadboard = (
    <div style={s.shell}>
      <div style={s.header} className="ui-tabs ui-tabs--editor">
        ...
        <div style={s.tabs}>
          <button
            className={`ui-tab ${selectedTab === 'breadboard' ? 'is-active' : ''} ${highlightedTabs?.includes('breadboard') ? 'pane-highlight-pulse' : ''}`}
            onClick={() => selectTab('breadboard')}
          >
            Breadboard
          </button>
          {hasCodeTab && (
            <button
              className={`ui-tab ${selectedTab === 'code' ? 'is-active' : ''} ${highlightedTabs?.includes('code') ? 'pane-highlight-pulse' : ''}`}
        ...
        <div style={s.actions}>
            ...
            <>
              <div style={s.zoomControls}>
                <button
                  ...
                  style={s.zoomBtn}
                  onClick={() =>
                    setZoomManually((prev) => clampBoardScale(prev - BOARD_SCALE_STEP))
                  }
                  aria-label="Zoom out"
                >
                  −
              ...
              <label style={s.wireColorToolbarField}>
                ...
                <select
                  ...
                  style={s.wireColorToolbarSelect}
                >
          ...
          {onCheck && (
            <button className="btn-secondary" style={s.actionBtn} onClick={onCheck}>
              Check Circuit
            </button>
          )}
          {onReset && (
            <button className="btn-ghost-outline" style={s.actionBtn} onClick={onReset}>
              Reset
            </button>
          )}
        </div>
      ...
      {selectedTab === 'code' && hasCodeTab ? (
        ...
        <div
          style={{
            ...s.workspace,
            gridTemplateColumns: [hasPalette && '216px', 'minmax(0, 1fr)', hasInspector && '230px']
              .filter(Boolean)
              .join(' '),
          }}
        >
          ...
          {hasPalette && (
            <div style={s.palette}>
              <p style={s.paletteHint}>
                Drag a part onto the board, then drag from one pin to another to wire them together.
              </p>
              {PALETTE.filter(([type]) => paletteTypes.includes(type)).map(([type, label]) => (
                <button
                  ...
                  disabled={readOnly}
                  draggable={!readOnly}
                  title={COMPONENT_DESCRIPTIONS[type] ?? label}
                  style={s.paletteBtn}
                  onClick={() => addComponent(type)}
                  onDragStart={(event) => handlePaletteDragStart(event, type)}
                >
                  <span style={s.paletteIcon}>
          ...
          >
            {hasShort && (
              <p style={s.shortWarning} role="status">
                <span aria-hidden="true" style={s.shortWarningMark}>
                  !
                </span>
                Short circuit - the highlighted wires connect the supply straight back to itself,
                with no part in between.
              </p>
            )}
            <div
              style={{
            ...
            >
              ...
              >
                <div
                  style={{ ...s.boardSurface, width: boardGridWidth, height: boardGridHeight }}
                />
                {Array.from({ length: boardRows }).map((_, row) =>
                  row < HIDDEN_EDGE_HOLE_ROWS || row >= boardRows - HIDDEN_EDGE_HOLE_ROWS
                    ? null
                ...
                >
                  {wireRoutes.map(({ wire, points, path }) => {
                    ...
                    const currentPath =
                      getWireCurrentDirection(circuit, wire) === 'reverse'
                    ...
                    return (
                      <g key={wire.id}>
                        ...
                        <path
                          d={path}
                          stroke={isSelected ? SELECTION_COLOUR : '#1f2937'}
                  ...
                  {drag?.type === 'wire' && pinPoint(drag.from) && (
                    <path
                      d={directWirePath(pinPoint(drag.from), drag.current)}
                      stroke="#0f172a"
                      strokeWidth="3"
                      strokeDasharray="8 7"
                      strokeLinecap="round"
                      ...
                      style={{
                        ...
                        borderColor: shortedComponentIds.has(component.id)
                          ...
                          : selectedId === component.id
                            ? 'var(--colour-primary)'
                            : state.on || state.powered || state.switched || state.conducting
                              ? PART_LIVE
                              : PART_BORDER,
                        ...
                        cursor:
                          readOnly || structureLocked
                            ? 'default'
                            : drag?.type === 'component' && drag.id === component.id
                              ? 'grabbing'
                              : 'grab',
                      }}
                      ...
                      {component.locked && (
                        <span
                          ...
                          title={
                            setupMode
                              ? 'Fixed for students'
                              : 'Fixed in place - you can still use its controls'
                          }
