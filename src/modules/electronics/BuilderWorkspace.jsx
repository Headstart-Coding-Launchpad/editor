...
export default function BuilderWorkspace({
  ...
  const activeCircuit = useMemo(() => {
    if (codeTab === 'complete')
      return parseCircuit(task.completeCircuit, task.starterCircuit ?? DEFAULT_CIRCUIT)
    if (stageMatch)
      return parseCircuit(
        codeStages?.[Number(stageMatch[1])]?.circuit,
        task.starterCircuit ?? DEFAULT_CIRCUIT
      )
    return parseCircuit(task.starterCircuit, DEFAULT_CIRCUIT)
  }, [codeTab, codeStages, task.completeCircuit, task.starterCircuit])
  const availableComponents = useMemo(
    () => normalizeAvailableComponents(task.availableComponents),
    [task.availableComponents]
  )
  const activeMicrocontroller =
    activeCircuit.components.find((component) => component.type === 'microcontroller') ?? null
  const microcontrollerPins = activeMicrocontroller
    ? normalizeMicrocontrollerPins(activeMicrocontroller.pins)
    : []
  const fixedMicrocontrollerPins = microcontrollerPins.filter(
    (pin) => !isMicrocontrollerSignalPin(pin)
  )
  ...
  const activeBoardSize = useMemo(() => {
    ...
    return (
      BOARD_SIZE_OPTIONS.find((option) => option.rows === rows && option.cols === cols)?.id ??
      'custom'
    )
  ...
  function updateAvailableComponents(type, enabled) {
    ...
    onUpdate({
      ...task,
      availableComponents: COMPONENT_TYPES.filter((componentType) => current.has(componentType)),
    })
  }
  ...
  function handleTestChecks() {
    ...
    const results = checksToEval.map((c) => ({
      ...c,
      passed: evaluateSingleCheck(c, '', { circuit: activeCircuit }),
    }))
  ...
  function updateLegacyMicrocontrollerCode(code) {
    onUpdate({
      ...task,
      microcontroller: { ...(task.microcontroller ?? {}), enabled: true, starterCode: code },
    })
  }
  ...
  function updateMicrocontroller(
    nextComponent,
    nextWires = activeCircuit.wires,
    nextControls = activeCircuit.controls
  ) {
    if (!activeMicrocontroller) return
    updateCircuit({
      ...activeCircuit,
      components: activeCircuit.components.map((component) =>
        component.id === activeMicrocontroller.id ? nextComponent : component
      ),
      wires: nextWires,
  ...
  function addGpioPin() {
    ...
    updateMicrocontroller({ ...activeMicrocontroller, pins }, activeCircuit.wires, {
  ...
  function removeGpioPin(pin) {
    ...
    const pins = normalizeMicrocontrollerPins(activeMicrocontroller.pins, []).filter(
      (item) => item !== pin
    )
    ...
    const gpio = { ...(control.gpio ?? {}) }
    delete gpio[pin]
    updateMicrocontroller({ ...activeMicrocontroller, pins }, nextWires, {
      ...(activeCircuit.controls ?? {}),
      [activeMicrocontroller.id]: { ...control, gpio },
    })
  }
      ...
      {activeStage && (
        <div style={s.stageMetadataBar}>
          ...
          <input
            ...
            onChange={(event) =>
              replaceStage(activeStageIndex, { ...activeStage, label: event.target.value })
            }
      ...
      {isSupportStage ? (
        ...
        <>
          ...
          <label style={s.boardSizeField}>
            ...
            <select
              value={activeBoardSize}
              onChange={(event) => updateBoardSize(event.target.value)}
              style={s.boardSizeSelect}
            >
              {activeBoardSize === 'custom' && (
                <option value="custom">
                  Custom ({activeCircuit.board?.rows ?? '?'} x {activeCircuit.board?.cols ?? '?'})
                </option>
              )}
              {BOARD_SIZE_OPTIONS.map((option) => (
                <option key={option.id} value={option.id}>
                  {option.label} ({option.rows} x {option.cols})
                </option>
              ))}
            </select>
          </label>
              ...
              <div style={s.gpioPanelHeader}>
                <div>
                  ...
                  <div style={s.fixedPins}>
                    {fixedMicrocontrollerPins.map((pin) => (
                      <span key={pin} style={s.fixedPin}>
                        {pin}
                      </span>
                    ))}
                  </div>
          ...
          <div style={s.componentPicker}>
            ...
            <div style={s.groupOptions} aria-label="Component groups">
              {COMPONENT_GROUPS.map((group) => {
                const enabledCount = group.types.filter((type) =>
                  availableComponents.includes(type)
                ).length
              ...
              {COMPONENT_TYPES.map((type) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...s.componentOption,
                      ...(enabled ? s.componentOptionOn : s.componentOptionOff),
                    }}
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...s.optionBadge,
                        ...(enabled ? s.optionBadgeOn : s.optionBadgeOff),
                      }}
                    >
                      {enabled ? 'On' : 'Off'}
                    </span>
          ...
          <p style={s.microHint}>
            Add a Micro Controller component to enable the MicroPython tab. Select the component to
            edit its GPIO pins.
          </p>
        </>
      )}
