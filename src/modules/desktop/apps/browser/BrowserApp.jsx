...
export default function BrowserApp({
  ...
  function pushLocation(next) {
    ...
    const withWindow = {
      ...state,
      windows: state.windows.map((w) => (w.id === win.id ? { ...w, ...windowPatch } : w)),
    }
    if (next.type === 'page') {
  ...
  function handleDownload(downloadPage) {
    ...
    onStateChange({
      ...state,
      fs: createEntry(fs, path, 'file', downloadPage.download.content ?? ''),
    })
  }
...
function SearchBox({ draft, onDraftChange, onSubmit, disabled }) {
  return (
    <form onSubmit={onSubmit} style={s.searchBoxForm}>
      ...
      />
      <button type="submit" className="btn-primary" style={s.searchBoxBtn} disabled={disabled}>
        Search
      </button>
    </form>
  )
        ...
        <ul style={s.resultList}>
          {results.map(({ pageId, page }) => (
            <li key={pageId} style={s.resultItem}>
              ...
              {page.searchable?.snippet && (
                <div style={s.resultSnippet}>{page.searchable.snippet}</div>
              )}
            </li>
          ))}
        </ul>
