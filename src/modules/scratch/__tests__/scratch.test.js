...
describe('broadcast timing', () => {
  ...
  it('event_broadcast does not await receivers — sender continues immediately', async () => {
    ...
    signal.onVariablesChange = (vars) => {
      Object.assign(signal.variables, vars)
      log.push('after_broadcast')
    }
    ...
    const onUpdate = () => {}
    signal.onVariablesChange = () => {
      afterBroadcastFired = true
    }
...
describe('addPredefinedBlocksToToolbox', () => {
  ...
  it('appends a block into the correct category in an XML toolbox', () => {
    const xml =
      '<xml><category name="Motion" colour="#4C97FF"><block type="motion_movesteps"/></category></xml>'
  ...
  it('creates the matching category for a predefined block whose type is not in the XML toolbox', () => {
    const xml =
      '<xml><category name="Events" colour="#FFAB19"><block type="event_whenflagclicked"/></category></xml>'
    const result = addPredefinedBlocksToToolbox(xml, predefined)
    const doc = new DOMParser().parseFromString(result, 'text/xml')
...
describe('addPrebuiltStacksToToolbox', () => {
  ...
  it('appends connected stacks to an XML toolbox', () => {
    const xml =
      '<xml><category name="Motion" colour="#4C97FF"><block type="motion_movesteps"/></category><category name="Looks" colour="#9966FF"><block type="looks_say"/></category></xml>'
    ...
    expect(
      added.querySelector('value[name="STEPS"] > shadow > field[name="NUM"]')?.textContent
    ).toBe('25')
  })
  ...
  it('appends stacks to a root-level XML toolbox', () => {
    ...
    const result = addPrebuiltStacksToToolbox(xml, [connectedStack])
    const doc = new DOMParser().parseFromString(result, 'text/xml')
    const blocks = Array.from(doc.documentElement.children).filter(
      (block) => block.tagName.toLowerCase() === 'block'
    )
  ...
  it('creates a missing category in a JSON toolbox for a stack whose block type is absent', () => {
    const noControl = {
      ...DEFAULT_TOOLBOX,
      contents: DEFAULT_TOOLBOX.contents.filter((c) => c.name !== 'Control'),
    }
  ...
  it('creates a missing category in an XML toolbox for a stack whose block type is absent', () => {
    ...
    const looksStack = {
      id: 'stack-3',
      stack: createScratchBlockStack('looks_say', { MESSAGE: 'Hi' }),
    }
...
describe('addCreateVariableButtonToToolbox', () => {
  it('inserts the button ahead of the existing Variables category contents in a JSON toolbox', () => {
    ...
    expect(varsCat.contents[0]).toEqual({
      kind: 'button',
      text: 'Make a Variable',
      callbackKey: CREATE_VARIABLE_CALLBACK_KEY,
    })
    expect(varsCat.contents.slice(1)).toEqual(
      DEFAULT_TOOLBOX.contents.find((c) => c.name === 'Variables').contents
    )
    // Original toolbox is untouched.
    expect(DEFAULT_TOOLBOX.contents.find((c) => c.name === 'Variables').contents[0].kind).toBe(
      'block'
    )
  })
  ...
  it('creates a Variables category when the toolbox has none (JSON)', () => {
      ...
      contents: [
        {
          ...
          name: 'Motion',
          colour: '#4C97FF',
          contents: [{ kind: 'block', type: 'motion_movesteps' }],
        },
      ],
    }
    ...
    expect(varsCat.contents).toEqual([
      { kind: 'button', text: 'Make a Variable', callbackKey: CREATE_VARIABLE_CALLBACK_KEY },
    ])
  })
  ...
  it('inserts the button into an existing Variables category in an XML toolbox', () => {
    const xml =
      '<xml><category name="Variables" colour="#FF8C1A"><block type="data_variable"/></category></xml>'
  ...
  it('creates a Variables category when an XML toolbox has none', () => {
    ...
    expect(category.querySelector('button')?.getAttribute('callbackkey')).toBe(
      CREATE_VARIABLE_CALLBACK_KEY
    )
  })
...
describe('buildAlwaysOpenToolbox', () => {
  it('adds editable shadow inputs to root-level XML blocks', () => {
    ...
    expect(
      doc.querySelector(
        'block[type="looks_sayforsecs"] value[name="MESSAGE"] shadow[type="text"] field[name="TEXT"]'
      )?.textContent
    ).toBe('Hello!')
    expect(
      doc.querySelector(
        'block[type="looks_sayforsecs"] value[name="SECS"] shadow[type="math_number"] field[name="NUM"]'
      )?.textContent
    ).toBe('2')
  })
})
...
function makeNum(value) {
  return {
    type: 'math_number',
    getFieldValue: (n) => (n === 'NUM' ? String(value) : null),
    getInputTargetBlock: () => null,
    getNextBlock: () => null,
  }
}
...
function makeText(value) {
  return {
    type: 'text',
    getFieldValue: (n) => (n === 'TEXT' ? String(value) : null),
    getInputTargetBlock: () => null,
    getNextBlock: () => null,
  }
}
...
describe('operator_letter_of', () => {
  it('returns the first letter', async () => {
    const op = makeReporter(
      'operator_letter_of',
      {},
      { LETTER: makeNum(1), STRING: makeText('hello') }
    )
    const { vars } = await runAndGetVars(makeSetVar('result', op))
  ...
  it('returns empty string for out-of-bounds index', async () => {
    const op = makeReporter(
      'operator_letter_of',
      {},
      { LETTER: makeNum(99), STRING: makeText('hi') }
    )
    const { vars } = await runAndGetVars(makeSetVar('result', op))
...
describe('operator_contains', () => {
  it('returns true when string contains substring', async () => {
    const op = makeReporter(
      'operator_contains',
      {},
      { STRING1: makeText('apple'), STRING2: makeText('pp') }
    )
    const { vars } = await runAndGetVars(makeSetVar('result', op))
  ...
  it('is case-insensitive', async () => {
    const op = makeReporter(
      'operator_contains',
      {},
      { STRING1: makeText('Apple'), STRING2: makeText('apple') }
    )
    const { vars } = await runAndGetVars(makeSetVar('result', op))
    expect(vars.result).toBe(true)
  })
  ...
  it('returns false when substring not found', async () => {
    const op = makeReporter(
      'operator_contains',
      {},
      { STRING1: makeText('apple'), STRING2: makeText('z') }
    )
    const { vars } = await runAndGetVars(makeSetVar('result', op))
...
describe('control_wait_until', () => {
  it('exits immediately when condition is already true', async () => {
    ...
    const conditionTrue = makeReporter(
      'operator_equals',
      {},
      {
        OPERAND1: makeText('a'),
...
describe('control_repeat_until', () => {
  it('never runs substack when condition is already true', async () => {
    ...
    const repeatBlock = makeStatement(
      'control_repeat_until',
      {},
      { CONDITION: conditionTrue, SUBSTACK: inner },
      marker
    )
  ...
  it('runs substack until condition becomes true via variable', async () => {
    ...
    const repeatBlock = makeStatement(
      'control_repeat_until',
      {},
      { CONDITION: condition, SUBSTACK: inner },
      marker
    )
...
describe('Scratch touching a sprite that has clones', () => {
  ...
  it('reports touching when only a clone of the target sprite overlaps, not the (distant) original', async () => {
    ...
    const signal = createRunSignal()
    signal.clones.set('rock__clone1', {
      id: 'rock__clone1',
      baseId: 'rock',
      workspace: null,
      state: makeState({ x: 0, y: 0 }),
      costumes: [],
    })
  ...
  it('ignores a hidden clone of the target sprite', async () => {
    ...
    signal.clones.set('rock__clone1', {
      ...
      baseId: 'rock',
      workspace: null,
      state: makeState({ x: 0, y: 0, visible: false }),
      costumes: [],
    })
