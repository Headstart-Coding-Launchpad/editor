...
export default function BuilderWorkspace({
  ...
  const previewAssets = useMemo(() => {
    const staticAssets = (lesson?.assets ?? []).map((name) => ({
      name,
  ...
  return (
    <div style={s.wrap}>
      ...
      {!isSupportStage && (
        <div style={s.preview}>
          <div style={s.previewHeader}>
            ...
            <button
              className={previewRunning ? 'btn-danger' : 'btn-primary'}
              style={{ padding: '6px 12px', fontSize: 12 }}
              onClick={() => {
                if (previewRunning) setPreviewRunning(false)
                else {
                  setPreviewCode(activePythonCode)
                  setPreviewRunning(true)
                  setRunId((id) => id + 1)
                }
              }}
            >
              {previewRunning ? 'Stop' : 'Run game'}
            </button>
          </div>
          <ArcadePreview
            code={previewCode}
            assets={previewAssets}
            tilemaps={previewTilemaps}
            runId={runId}
      ...
      {!isSupportStage && (
        <div style={s.design}>
          <label style={s.toolsLabel}>
            Student design tools{' '}
            <select
              value={task.arcadeTools ?? 'none'}
              onChange={(event) => onUpdate({ ...task, arcadeTools: event.target.value })}
            >
              <option value="none">None</option>
              <option value="sprites">Sprites only</option>
              <option value="tilemaps">Tilemaps only</option>
              <option value="both">Sprites and tilemaps</option>
            </select>
          </label>
          <ArcadeDesignStudio
            task={null}
            design={designForCodeTab(task, codeTab)}
            onChange={updateDesign}
            availableAssets={externalAssets}
            layout="builder"
          />
        </div>
      )}
...
const s = {
  ...
  design: { gridColumn: '1 / -1', minWidth: 0 },
  toolsLabel: {
    display: 'flex',
    alignItems: 'center',
    gap: 6,
