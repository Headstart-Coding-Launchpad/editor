...
export default function ArcadeDesignStudio({ task, design, onChange, availableAssets = [], readOnly = false, title = 'Game art & level', layout = 'default' }) {
  ...
  function copyFrame(frameIndex) {
    ...
    updateSprite({ ...sprite, frames: sprite.frames.flatMap((frame, index) => index === frameIndex ? [frame, copied] : [frame]) })
  }
  ...
  function deleteTile(symbol = selectedSymbol) {
    ...
    updateMap({
      ...map,
      tiles: Object.fromEntries(Object.entries(map.tiles).filter(([tileSymbol]) => tileSymbol !== symbol)),
      rows: map.rows.map(row => [...row].map(tileSymbol => tileSymbol === symbol ? '.' : tileSymbol).join('')),
    })
  ...
  return (
    <section style={s.section}>
      ...
      <div style={s.assets}>
        ...
        {generatedAssets.map(asset => <span key={asset.name} style={s.assetButton}><img src={asset.url} alt="" style={s.assetThumb} />{shortAssetName(asset.name)} <small>made here</small></span>)}
        {generatedTilemaps.map(tilemap => <span key={tilemap.name} style={s.mapAssetButton}>{tilemap.name}</span>)}
      ...
      <div style={builderLayout ? s.stacked : s.columns}>
        {canDrawSprites && (!tabbedLayout || activeTab === 'sprites') && <details open style={builderLayout ? s.collapsible : s.openPanel}>
          ...
          {sprite ? (
            <>
              ...
              <div style={s.spriteToolbar}><div style={s.colourPalette} aria-label="16 colour palette">{ARCADE_PALETTE.map(item => <button key={item.name} type="button" title={item.name.replace('_', ' ')} aria-label={`Use ${item.name.replace('_', ' ')}`} disabled={readOnly} onClick={() => setColour(item.hex)} style={{ ...s.colourSwatch, background: item.hex, ...(colour === item.hex ? s.colourSwatchActive : {}) }} />)}</div><label style={s.sizeField}>Size<select value={`${sprite.width}x${sprite.height}`} disabled={readOnly} onChange={event => resizeSprite(Number(event.target.value.split('x')[0]))}><option value="8x8">8 x 8</option><option value="16x16">16 x 16</option></select></label><span style={s.dimension}>{sprite.width} x {sprite.height} px</span>{!readOnly && <button className="btn-ghost" style={s.smallButton} onClick={addFrame}>+ Frame</button>}</div>
              <div style={s.frames}>
                {sprite.frames.map((frame, frameIndex) => <div key={frameIndex} style={s.frameWrap}><div style={s.frameHeading}><span style={s.frameLabel}>Frame {frameIndex + 1}</span>{!readOnly && <div style={s.frameActions}><button type="button" style={s.frameCopy} onClick={() => copyFrame(frameIndex)}>Copy</button>{sprite.frames.length > 1 && <button type="button" style={s.frameDelete} onClick={() => deleteFrame(frameIndex)}>Delete</button>}</div>}</div><div style={{ ...s.pixelGrid, gridTemplateColumns: `repeat(${sprite.width}, 22px)` }}>
        ...
        {canDrawMaps && (!tabbedLayout || activeTab === 'tilemaps') && <details open style={builderLayout ? s.collapsible : s.openPanel}>
          ...
          {map ? (
            <>
              <div style={s.row}><select value={map.id} onChange={event => setSelectedMapId(event.target.value)} style={s.select}>{normal.maps.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
              ...
              <div style={s.mapDimensions}><label style={s.field}>Width<input type="number" min="1" max="64" value={mapColumns} disabled={readOnly} onChange={event => setMapColumns(event.target.value)} /></label><label style={s.field}>Height<input type="number" min="1" max="64" value={mapRows} disabled={readOnly} onChange={event => setMapRows(event.target.value)} /></label>{!readOnly && <button type="button" className="btn-ghost" style={s.smallButton} onClick={resizeMap}>Resize map</button>}</div>
              <div style={s.tilePalette}><button type="button" onClick={() => setSelectedSymbol('.')} style={{ ...s.paletteTile, ...(selectedSymbol === '.' ? s.paletteTileActive : {}) }}>Empty</button>{Object.entries(map.tiles).map(([symbol, tile]) => <div key={symbol} style={s.paletteTileGroup}><button type="button" onClick={() => setSelectedSymbol(symbol)} style={{ ...s.paletteTile, ...(selectedSymbol === symbol ? s.paletteTileActive : {}) }} title={`Use ${tile.asset}`}><span style={s.paletteSymbol}>{symbol}</span>{assetUrls.get(tile.asset) && <img src={assetUrls.get(tile.asset)} alt="" style={s.paletteImage} />}</button>{!readOnly && <button type="button" style={s.tileDelete} onClick={() => deleteTile(symbol)} aria-label={`Remove tile ${symbol}`}>×</button>}</div>)}{!readOnly && <button className="btn-ghost" style={s.smallButton} onClick={addTile} disabled={!assetNames.length}>+ Tile</button>}</div>
              {selectedTile && <><div style={s.tileSettings}><label style={s.field}>Image<select value={selectedTile.asset} disabled={readOnly} onChange={event => selectTileAsset(event.target.value)}>{assetNames.map(name => <option key={name}>{name}</option>)}</select></label><label style={s.check}><input type="checkbox" checked={!!selectedTile.properties?.solid} disabled={readOnly} onChange={event => updateTile({ properties: { ...selectedTile.properties, solid: event.target.checked } })} /> Solid</label><label style={s.field}>Tag<input value={selectedTile.properties?.tag ?? ''} disabled={readOnly} placeholder="e.g. hazard" onChange={event => updateTile({ properties: { ...selectedTile.properties, tag: event.target.value } })} /></label><label style={s.field}>Property<input value={propertyKey} disabled={readOnly} onChange={event => setPropertyKey(event.target.value)} /></label><label style={s.field}>Value<input value={propertyValue} disabled={readOnly} placeholder="true, 10, lava" onChange={event => setPropertyValue(event.target.value)} /></label><button type="button" className="btn-ghost" style={s.smallButton} disabled={readOnly} onClick={addTileProperty}>Add</button></div>{Object.entries(selectedTile.properties ?? {}).filter(([key]) => key !== 'solid' && key !== 'tag').length > 0 && <div style={s.propertyList}>{Object.entries(selectedTile.properties).filter(([key]) => key !== 'solid' && key !== 'tag').map(([key, value]) => <span key={key} style={s.propertyChip}>{key}: {String(value)}{!readOnly && <button type="button" style={s.chipDelete} onClick={() => deleteTileProperty(key)} aria-label={`Delete ${key} property`}>Remove</button>}</span>)}</div>}</>}
...
const s = {
  ...
  row: { display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }, field: { display: 'flex', flexDirection: 'column', gap: 3, flex: 1, color: '#475569', fontWeight: 600 }, fileHint: { color: '#64748b', fontWeight: 400, fontSize: 10 }, check: { display: 'flex', alignItems: 'center', gap: 4, color: '#475569', whiteSpace: 'nowrap', fontWeight: 600 }, spriteToolbar: { display: 'flex', alignItems: 'end', flexWrap: 'wrap', gap: 10, padding: '6px 0 2px' }, colourPalette: { display: 'grid', gridTemplateColumns: 'repeat(8, 18px)', gap: 3 }, colourSwatch: { width: 18, height: 18, padding: 0, border: '1px solid #64748b', borderRadius: 3, cursor: 'crosshair' }, colourSwatchActive: { outline: '2px solid #6d28d9', outlineOffset: 1 }, sizeField: { display: 'flex', flexDirection: 'column', gap: 3, color: '#475569', fontWeight: 600 }, dimension: { paddingBottom: 5, color: '#64748b', fontSize: 11 },
  frames: { display: 'flex', gap: 10, overflowX: 'auto', padding: '3px 2px 6px' }, frameWrap: { display: 'flex', flexDirection: 'column', gap: 5 }, frameHeading: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }, frameLabel: { color: '#64748b', fontSize: 10, fontWeight: 700 }, frameActions: { display: 'flex', gap: 4 }, frameCopy: { padding: '2px 5px', color: '#4338ca', border: '1px solid #a5b4fc', borderRadius: 4, background: '#eef2ff', fontSize: 10, fontWeight: 700, cursor: 'pointer' }, frameDelete: { padding: '2px 5px', color: '#be123c', border: '1px solid #fda4af', borderRadius: 4, background: '#fff1f2', fontSize: 10, fontWeight: 700, cursor: 'pointer' }, pixelGrid: { display: 'grid', width: 'max-content', gap: 0, padding: 0, border: '1px solid #94a3b8', borderRadius: 0, background: '#f8fafc' }, pixel: { width: 22, height: 22, aspectRatio: '1 / 1', boxSizing: 'border-box', padding: 0, border: '1px solid #cbd5e1', borderRadius: 0, cursor: 'crosshair', touchAction: 'none', boxShadow: 'none' }, emptyPixel: { backgroundColor: '#fff', backgroundImage: 'linear-gradient(45deg, #e2e8f0 25%, transparent 25%), linear-gradient(-45deg, #e2e8f0 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #e2e8f0 75%), linear-gradient(-45deg, transparent 75%, #e2e8f0 75%)', backgroundSize: '8px 8px', backgroundPosition: '0 0, 0 4px, 4px -4px, -4px 0' },
  mapDimensions: { display: 'flex', alignItems: 'end', gap: 7, padding: 7, background: '#f8fafc', border: '1px solid #ddd6fe', borderRadius: 8 }, tileSettings: { display: 'grid', gridTemplateColumns: 'minmax(85px,1fr) auto minmax(70px,1fr) minmax(65px,1fr) minmax(65px,1fr) auto', gap: 6, alignItems: 'end', padding: 7, background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 8 }, propertyList: { display: 'flex', flexWrap: 'wrap', gap: 5, padding: '0 2px' }, propertyChip: { display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 5px', border: '1px solid #c7d2fe', borderRadius: 5, background: '#eef2ff', color: '#4338ca', fontSize: 11, fontWeight: 700 }, chipDelete: { padding: 0, border: 0, background: 'transparent', color: '#be123c', fontSize: 14, lineHeight: 1, cursor: 'pointer' }, tilePalette: { display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center', padding: '5px 0' }, paletteTileGroup: { display: 'flex', alignItems: 'center', gap: 2 }, paletteTile: { display: 'inline-flex', alignItems: 'center', gap: 4, height: 30, padding: '3px 6px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', color: '#475569', cursor: 'pointer' }, paletteTileActive: { border: '2px solid #6d28d9', background: '#f5f3ff', color: '#4c1d95' }, tileDelete: { width: 18, height: 22, padding: 0, border: '1px solid #fda4af', borderRadius: 4, background: '#fff1f2', color: '#be123c', fontSize: 15, fontWeight: 700, cursor: 'pointer' }, paletteSymbol: { fontFamily: 'var(--font-code)', fontWeight: 700 }, paletteImage: { width: 18, height: 18, imageRendering: 'pixelated' }, mapHint: { color: '#64748b', fontSize: 11, marginTop: 2 }, mapGrid: { display: 'grid', width: 'max-content', maxWidth: '100%', overflow: 'auto', gap: 2, padding: 5, border: '1px solid #c4b5fd', borderRadius: 9, background: '#ede9fe' }, cell: { width: 26, height: 26, aspectRatio: '1 / 1', boxSizing: 'border-box', padding: 2, border: '1px solid #a5b4fc', borderRadius: 5, fontSize: 9, color: '#334155', cursor: 'cell', overflow: 'hidden', touchAction: 'none' }, cellImage: { display: 'block', width: '100%', height: '100%', objectFit: 'contain', imageRendering: 'pixelated', pointerEvents: 'none' },
