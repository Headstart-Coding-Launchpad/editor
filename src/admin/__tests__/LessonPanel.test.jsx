...
vi.mock('firebase/firestore', () => ({
  ...
  doc: vi.fn((_db, col, id, subcol, subid) => ({
    __collection: col,
    __id: id,
    __subcollection: subcol,
    __subid: subid,
  })),
}))
...
async function openFirstLevel(user) {
  const levelToggle = screen
    .getAllByRole('button', { name: /lesson/i })
...
describe('LessonPanel', () => {
  ...
  it('collapses class forks beneath their stock lesson until expanded', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Intro to Python - Maple/i })
  ...
  it('blocks uploaded lessons that fail builder validation', async () => {
    ...
    expect(alertSpy).toHaveBeenCalledWith(
      expect.stringContaining('Cannot upload lesson until these validation errors are fixed')
    )
