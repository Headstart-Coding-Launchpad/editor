  ...
  return (
    <section style={s.section}>
      <div style={s.titleRow}>
        ...
        <p style={s.subtitle}>
          Files uploaded here are available to all lessons of that type in the builder. Scratch
          default sprites are used as the starting sprite config for new Scratch tasks.
        </p>
      </div>
...
function TypeAssetsEditor({ lessonType }) {
  ...
  useEffect(() => {
    return onSnapshot(
      ...
      (snap) => {
        setTypeData(snap.exists() ? snap.data() : {})
        setLoading(false)
      },
      () => setLoading(false)
    )
  ...
  function uploadFile(file) {
    ...
    task.on(
      ...
      async () => {
        ...
        await updateField('storageAssets', [
          ...current.filter((a) => a.name !== file.name),
          { name: file.name, url, showInEditor: false },
        ])
      }
    )
  }
  ...
  async function handleDelete(asset) {
    ...
    await updateField(
      'storageAssets',
      storageAssets.filter((a) => a.name !== asset.name)
    )
  }
  ...
  async function handleToggleShowInEditor(asset) {
    await updateField(
      'storageAssets',
      storageAssets.map((a) =>
        a.name === asset.name ? { ...a, showInEditor: !a.showInEditor } : a
      )
    )
  }
      ...
      <div style={s.sectionCard}>
        ...
        {storageAssets.map((asset) => (
          <div key={asset.name} style={s.assetRow}>
            ...
            <button style={s.removeBtn} onClick={() => handleDelete(asset)} title="Delete">
              ×
            </button>
          </div>
        ))}
...
function DefaultSpritesEditor({ sprites, storageAssets, onChange }) {
  ...
  function addSprite() {
    ...
    let id = `sprite${next}`
    while (sprites.some((sp) => sp.id === id)) {
      id = `sprite${parseInt(id.replace('sprite', '')) + 1}`
    }
    onChange([
      ...sprites,
      {
  ...
  function removeSprite(id) {
    ...
    if (
      !window.confirm(
        `Delete "${sprite?.name ?? 'this sprite'}" (and all its costumes) from the shared default sprites?`
      )
    )
      return
  ...
  function setVisualMode(id, mode) {
    ...
    const costumeKind =
      sp?.costumes?.[0]?.image !== undefined
        ? 'image'
        : sp?.costumes?.[0]?.emoji !== undefined
          ? 'emoji'
          : null
  ...
  function addCostume(spriteId) {
    ...
    const newCostume =
      mode === 'emoji'
        ? { name: `costume${next}`, emoji: '' }
        : { name: `costume${next}`, image: '' }
    onChange(
      sprites.map((s) =>
        s.id === spriteId ? { ...s, costumes: [...(s.costumes ?? []), newCostume] } : s
      )
    )
  }
  ...
  function removeCostume(spriteId, idx) {
    ...
    if (
      !window.confirm(
        `Delete costume "${costumeName ?? idx + 1}" from "${sprite?.name ?? 'this sprite'}"?`
      )
    )
      return
    onChange(
      sprites.map((s) =>
        s.id === spriteId ? { ...s, costumes: (s.costumes ?? []).filter((_, i) => i !== idx) } : s
      )
    )
  }
  ...
  return (
    <div style={s.sectionCard}>
      ...
      <p style={s.muted}>
        These sprites appear in the &ldquo;Add sprite&rdquo; picker in the lesson builder. Teachers
        can add any of them to a task. New tasks always start with a single Cat sprite; these are
        additional options teachers can choose from.
      </p>
          ...
          <div key={sp.id} style={s.spriteBlock}>
            <div style={s.spriteRow}>
              ...
              <div className="te-sprite-mode-sel">
                <button
                  type="button"
                  className={`te-sprite-mode-btn${mode === 'preset' ? ' te-sprite-mode-btn--active' : ''}`}
                  onClick={() => setVisualMode(sp.id, 'preset')}
                >
                  Shape
                </button>
                <button
                  type="button"
                  className={`te-sprite-mode-btn${mode === 'emoji' ? ' te-sprite-mode-btn--active' : ''}`}
                  onClick={() => setVisualMode(sp.id, 'emoji')}
                >
                  Emoji
                </button>
                <button
                  type="button"
                  className={`te-sprite-mode-btn${mode === 'costume' ? ' te-sprite-mode-btn--active' : ''}`}
                  onClick={() => setVisualMode(sp.id, 'costume')}
                >
                  Costume
                </button>
              </div>
              ...
              <div style={s.costumeSection}>
                {sp.costumes.map((c, idx) => (
                  <div key={idx} style={s.costumeRow}>
                    ...
                    <button
                      style={s.removeBtn}
                      onClick={() => removeCostume(sp.id, idx)}
              ...
              <div style={s.costumeSection}>
                {(sp.costumes ?? []).length === 0 && (
                  <p style={s.mutedSmall}>
                    No costumes yet. Add one to use an image from the shared assets above.
                  </p>
                )}
                {(sp.costumes ?? []).map((c, idx) => (
                  <div key={idx} style={s.costumeRow}>
...
function DefaultBackdropsEditor({ backdrops, storageAssets, onChange }) {
  function addBackdrop() {
    ...
    while (backdrops.some((b) => b.id === id)) {
      id = `backdrop${parseInt(id.replace('backdrop', '')) + 1}`
    }
  ...
  function removeBackdrop(id) {
    ...
    if (
      !window.confirm(
        `Delete "${backdrop?.name ?? 'this backdrop'}" from the shared default backdrops?`
      )
    )
      return
  ...
  return (
    <div style={s.sectionCard}>
      ...
      <p style={s.muted}>
        These backdrops appear in the &ldquo;Add backdrop&rdquo; picker offered to students (when a
        lesson author turns that on for a task) and can be reused across lessons.
      </p>

      {backdrops.length === 0 && <p style={s.muted}>No default backdrops configured yet.</p>}
