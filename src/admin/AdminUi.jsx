      ...
      <thead>
        <tr>
          {headers.map((header, index) => (
            <th key={header || `col-${index}`} style={styles.th}>{header}</th>
          ))}
        </tr>
      </thead>
...
export function AdminMessage({ tone = 'muted', children }) {
  return <p style={{ ...styles.message, ...(messageStyles[tone] || messageStyles.muted) }}>{children}</p>
}
...
export const adminUiStyles = {
  ...
  linkText: {
    fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.9rem',
    color: 'var(--colour-primary)', textDecoration: 'none',
  },
}
...
const styles = {
  ...
  title: {
    fontFamily: 'var(--font-title)', fontWeight: 700, fontSize: '1.1rem',
