...
import {
  IMAGE_EXTENSIONS,
  SCRATCH_BLOCK_CATEGORIES,
  getCodeBlockOptions,
  getInlineCodeOptions,
  isInsideScratchCodeBlock,
} from './markdown/editorOptions'

export { getInlineCodeOptions } from './markdown/editorOptions'
...
export function MarkdownFieldEditor({
  ...
  function applyFormat(action) {
    ...
    if (action === 'bold') {
      ...
      if (isInsideScratchCodeBlock(before)) {
        const needNewline = start > 0 && val[start - 1] !== '\n'
        const toInsert = (needNewline ? '\n' : '') + blockText
        newVal = before + toInsert + after
        cursorStart = start + toInsert.length
        cursorEnd = cursorStart
      } else {
...
export function MarkdownToolbar({
  ...
  const codeBlockOptions = getCodeBlockOptions(lessonType)
