...
function validateTaskShape(task, label, draft, errors) {
  ...
  if (!draft && task.intent != null && typeof task.intent !== 'string')
    errors.push(`${label} intent must be a Markdown string when provided`)
  if (task.taskActivity != null && typeof task.taskActivity !== 'string')
    errors.push(`${label} taskActivity must be a string when provided`)
  if (task.quizType != null && !QUIZ_TYPES.has(task.quizType))
    errors.push(`${label} has an invalid quiz type`)
  for (const field of [
    'options',
    'pairs',
    'blanks',
...
export function validateDraftLessonStructure(lesson, errors) {
  if (lesson.draft != null && typeof lesson.draft !== 'boolean')
    errors.push('draft must be a boolean when provided')
  if (lesson.version != null && (!Number.isInteger(lesson.version) || lesson.version < 0))
    errors.push('version must be a non-negative integer when provided')
  ...
  lesson.tasks.forEach((item, index) => {
    ...
    if (item.type === 'group') {
      if (typeof item.title !== 'string' || !item.title.trim())
        errors.push(`Group ${index + 1} is missing a title`)
      if (!Array.isArray(item.subtasks)) errors.push(`Group ${index + 1} subtasks must be an array`)
      else
        item.subtasks.forEach((task, subIndex) =>
          validateTaskShape(
            task,
            `Task ${index + 1}.${subIndex + 1}`,
            lesson.draft === true,
            errors
          )
        )
