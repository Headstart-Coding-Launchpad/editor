...
describe('mergeStorageAssets', () => {
  it('uses the Firebase Storage folder as the inventory and defaults discovered files into the web editor', () => {
    expect(
      mergeStorageAssets([], [{ name: 'hero.png', url: 'https://storage.test/hero.png' }])
    ).toEqual([{ name: 'hero.png', url: 'https://storage.test/hero.png', showInEditor: true }])
  })
  ...
  it('preserves schema metadata for files that still exist in Firebase Storage', () => {
    ...
    ).toEqual([{ name: 'hero.png', url: 'new-url', showInEditor: false }])
  })
  ...
  it('keeps schema-only assets as a compatibility fallback', () => {
    ...
    ).toEqual([{ name: 'legacy.png', url: 'https://example.test/legacy.png', showInEditor: true }])
  })
})
