...
describe('composed lesson helpers', () => {
  it('resolves the active module from a task', () => {
    expect(getTaskContext(lesson, 3)).toMatchObject({
      moduleType: 'python',
      lessonModule: { id: 'python' },
    })
      ...
      tasks: [
        {
          ...
          moduleId: 'html-intro',
          title: 'Intro page',
          starterFiles: [{ name: 'index.html', content: '<h1>One</h1>' }],
        },
        {
          id: 2,
          moduleId: 'html-finale',
          title: 'Final page',
          starterFiles: [{ name: 'index.html', content: '<h1>Two</h1>' }],
        },
      ],
    }

    expect(getLessonModules(repeatedHtml).map((module) => module.id)).toEqual([
      'html-intro',
      'html-finale',
    ])
    expect(getTaskContext(repeatedHtml, 2).lessonModule).toMatchObject({
      id: 'html-finale',
      type: 'html',
    })
    expect(getEffectiveLessonForTask(repeatedHtml, 2).sandboxStarterFiles[0].content).toContain(
      'Two'
    )
  ...
  it('rejects code tasks with no selected lesson module', () => {
    expect(
      validateComposedStructure({ type: 'composed', tasks: [{ id: 1, title: 'Loose code' }] })
    ).toHaveLength(1)
  })
})
