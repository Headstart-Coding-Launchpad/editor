...
export default function TeacherReportsPanel({ lessonId, onClose }) {
  ...
  useEffect(() => {
    ...
    fetchSessionReports(lessonId)
      .then(items => { if (!cancelled) { setReports(items); setLoading(false) } })
        ...
        <div style={s.body}>
          ...
          {reports.length > 0 && (
            <table style={s.table}>
              <thead>
                <tr>
                  {['Started', 'Ended', 'Students', ''].map(h => <th key={h} style={s.th}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {reports.map(r => (
...
const s = {
  ...
  modal: {
    background: '#fff', borderRadius: 12, width: 'min(700px, 92vw)', maxHeight: '85vh',
  ...
  table: { width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-body)', fontSize: '0.88rem' },
  th: {
