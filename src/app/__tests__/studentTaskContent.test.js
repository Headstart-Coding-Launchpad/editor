...
describe('selectPythonTaskCode', () => {
  ...
  it('prefers own solo work before carried or starter code', () => {
    const readSavedCode = vi.fn((id) =>
      id === 2 ? { code: 'own code' } : { code: 'carried code' }
    )
    expect(
      selectPythonTaskCode({ tasks: groupedTasks, task, taskId: 2, phase: 'solo', readSavedCode })
    ).toBe('own code')
  ...
  it('walks from a later group through a skipped bridge task to earlier saved code', () => {
    const tasks = [
      ...
      },
      {
        id: 'project',
        type: 'group',
        subtasks: [{ id: 21, starterCode: 'task 21 starter', carryCodeFrom: 15 }],
      },
    ]
  ...
  it('carries saved code directly across groups', () => {
    const tasks = [
      { id: 'source-group', type: 'group', subtasks: [{ id: 15, starterCode: 'source' }] },
      {
        id: 'target-group',
        type: 'group',
        subtasks: [{ id: 21, starterCode: 'target', carryCodeFrom: 15 }],
      },
    ]
...
describe('selectHtmlTaskFiles', () => {
  ...
  it('selects saved files independently and does not mutate starter files', () => {
    const files = selectHtmlTaskFiles({
      ...
      taskId: 2,
      phase: 'solo',
      readSavedFile: (id, name) =>
        id === 2 && name === 'index.html' ? 'own html' : id === 1 ? 'carried css' : null,
    })
  ...
  it('falls back from saved task blocks to carried blocks and starter blocks', () => {
    expect(
      selectScratchInitialProject({
        ...
        taskId: 2,
        readSavedCode: (id) =>
          id === 2 ? { state: { selected: 'own' } } : { state: { selected: 'carry' } },
      })
    ...
    expect(selectScratchInitialProject({ task, taskId: 2, readSavedCode: () => null })).toBe(
      starterBlocks
    )
  })
  ...
  it('supports non-code saved state for filesystem and electronics carry fields', () => {
    ...
    expect(
      resolveSavedCarrySource({
        ...
        readSavedState: (id) =>
          id === 1 ? { '/': { type: 'dir' }, '/main.py': { type: 'file', content: '' } } : null,
