...
export async function listLessonFeedback(
  lessonId,
  { taskId = null, scope = null, includeArchived = false } = {}
) {
...
export async function listAllFeedback({
  ...
  const [platform, lesson] = await Promise.all([
    ...
    lessonId
      ? listLessonFeedback(lessonId, { taskId, scope, includeArchived })
      : listAllLessonFeedback({ taskId, scope, includeArchived }),
  ])
...
export async function addLessonFeedback(
  lessonId,
  { text, teacherEmail = '', taskId = null, taskTitle = null, lessonTitle = null } = {}
) {
  if (!lessonId) throw new Error('lessonId is required')
...
export async function addPlatformFeedback({
  text,
  teacherEmail = '',
  lessonId = null,
  lessonTitle = null,
...
export async function archiveLessonFeedbackItem(lessonId, feedbackId) {
  ...
  if (!snap.exists)
    throw new Error(`Feedback item "${feedbackId}" not found under lesson "${lessonId}"`)
