  ...
  flat.forEach((task, i) => {
    ...
    for (const field of ['carryCodeFrom', 'carryBlocksFrom', 'carryFsFrom', 'carryCircuitFrom']) {
      if (
        task[field] != null &&
        allowedCarrySources &&
        !allowedCarrySources.includes(task[field])
      ) {
    ...
    if (task.taskType === 'quiz') {
      ...
      if (quizType === 'multiple_choice') {
        ...
        if (task.options?.some((o) => !o.text?.trim()))
        ...
        if (task.pairs?.some((p) => !p.prompt?.trim() || !p.answer?.trim()))
        ...
        if (task.blanks?.some((b) => !b.answer?.trim()))
      ...
      if (task.check?.type === 'sprite_property') {
        ...
        if (task.check.value == null || task.check.value === '')
          errors.push(`Task ${n} sprite check is missing a value`)
      }
