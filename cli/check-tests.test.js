...
const lesson = {
  ...
  tasks: [{
    ...
    feedbackChecks: [
      ...
      { type: 'code', operator: 'contains', value: 'pass', hint: 'Replace pass with your solution.' },
    ],
  }],
}
...
describe('lesson check cases', () => {
  it('uses runtime code normalisation for whitespace, string literals, and alternate valid solutions', () => {
    ...
    expect(result.cases[2].matchedFeedback).toEqual([expect.objectContaining({
      index: 2,
  ...
  it('accepts the documented YAML cases-file shape', () => {
    const casesFile = parseJsonOrYaml('check-cases.yaml', `
tasks:
  - id: 4
    cases:
      - name: YAML case
  ...
  it('reports feedback and legacy incorrect checks that match a code case', () => {
    const feedbackLesson = {
      ...
      tasks: [{
        ...
        incorrectChecks: [{ type: 'code', operator: 'contains', value: 'while', hint: 'Use a for loop instead.' }],
      }],
    }
    ...
    expect(result.success).toBe(true)
    expect(result.cases[0].matchedFeedback).toEqual([expect.objectContaining({
      index: 1,
